...
5. **Return the problem file link** using the **absolute path** pointing to the current round's working copy (e.g. `<project_root>/my-progress/round-N/problems/<category>/<problem>/<problem>.go`), **never** the `templates/` path.
